...
import { TONS_SELECT, normalizarTom } from '../lib/pitch'
...
export default function Admin() {
  ...
  async function criarMusica(e) {
    ...
    let tomNormalizado = null
    if (tomInput.trim()) {
      tomNormalizado = normalizarTom(tomInput.trim())
      if (!tomNormalizado) {
        setTomErro('Tom inválido. Ex: C, F#, Bb, D#/Eb')
        return
      }
    }
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'start' }}>
        ...
        <div className="fade-in">
          ...
          <form onSubmit={criarMusica} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20, marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {/* Tom */}
            <div>
              <label style={{ fontSize: 11, color: 'var(--text2)', letterSpacing: 2, textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Tom original (opcional)</label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
                {TONS_SELECT.map(t => (
                  <button key={t} type="button" onClick={() => { setTomInput(t); setTomErro('') }} style={{
                    padding: '4px 10px', borderRadius: 'var(--radius)', fontSize: 12,
                    background: tomInput === t ? 'var(--accent)' : 'var(--bg3)',
                    color: tomInput === t ? '#000' : 'var(--text2)',
                    border: `1px solid ${tomInput === t ? 'var(--accent)' : 'var(--border)'}`,
                    cursor: 'pointer', fontFamily: 'var(--font-mono)',
                  }}>{t}</button>
                ))}
              </div>
              {tomErro && <div style={{ fontSize: 12, color: 'var(--danger)', marginTop: 4 }}>{tomErro}</div>}
              {tomInput && (
                <div style={{ fontSize: 12, color: 'var(--accent)', marginTop: 4 }}>
                  Tom selecionado: <strong>{tomInput}</strong>
                  <button type="button" onClick={() => setTomInput('')} style={{ background: 'none', color: 'var(--text3)', fontSize: 12, marginLeft: 8, cursor: 'pointer' }}>✕ limpar</button>
                </div>
              )}
            </div>
            ...
            {musicas.map(m => (
              <div key={m.id} onClick={() => selecionarMusica(m)} style={{
                ...
                <button onClick={e => { e.stopPropagation(); deletarMusica(m.id) }} style={{
                  background: 'none', color: 'var(--text3)', fontSize: 16, padding: 4, transition: 'color 0.15s', cursor: 'pointer', border: 'none',
                }}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    {arquivos.map((f, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg3)', borderRadius: 'var(--radius)', padding: '8px 12px', fontSize: 12 }}>
                        ...
                        <span style={{ flex: 1, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.name.replace(/\.[^.]+$/, '').replace(/[-_]/g, ' ')}</span>
