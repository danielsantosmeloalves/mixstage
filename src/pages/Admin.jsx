              ...
              <form onSubmit={uploadTrilhas} style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:20, marginBottom:20, display:'flex', flexDirection:'column', gap:12 }}>
                ...
                {arquivos.length > 0 && (
                  <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                    <div style={{ fontSize:11, color:'var(--text2)' }}>{arquivos.length} arquivo(s) prontos:</div>
                    {arquivos.map((f, i) => (
                ...
                {msg && (
                  <div style={{ padding:'8px 12px', borderRadius:'var(--radius)', fontSize:12, background: msg.includes('erro') || msg.includes('Erro') ? 'rgba(255,68,68,0.1)' : 'rgba(60,255,143,0.1)', color: msg.includes('erro') || msg.includes('Erro') ? 'var(--danger)' : 'var(--success)' }}>{msg}</div>
                )}
