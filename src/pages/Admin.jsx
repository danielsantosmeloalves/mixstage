...
// Comprime áudio para mono 22050Hz antes do upload (~50% menor)
async function comprimirAudio(file) {
  try {
    ...
    // Cria buffer mono mixando os canais
    const monoBuf = offCtx.createBuffer(1, decoded.length, decoded.sampleRate)
    const monoData = monoBuf.getChannelData(0)
    const ch = decoded.numberOfChannels
    for (let i = 0; i < decoded.length; i++) {
      let sum = 0
      for (let c = 0; c < ch; c++) sum += decoded.getChannelData(c)[i]
      monoData[i] = sum / ch
    }
    ...
    // Converte para WAV
    const numSamplesOut = rendered.length
    const dataSize = numSamplesOut * 2
    ...
    return new File([new Blob([buf], { type: 'audio/wav' })],
...
export default function Admin() {
  ...
  async function uploadTrilhas(e) {
    ...
    for (let i = 0; i < arquivos.length; i++) {
      setMsg(`Comprimindo trilha ${i + 1} de ${arquivos.length}...`)
      const arquivoComprimido = await comprimirAudio(arquivos[i])
      const ext = 'wav'
      const path = `${musicaSelecionada.id}/${Date.now()}-${i}.${ext}`
      const { error: upErr } = await supabase.storage.from('audio').upload(path, arquivoComprimido)
