...
export default function Admin() {
  ...
  async function salvarCifra(e) {
    e.preventDefault()
    if (!cifrasTom || !cifraTexto.trim() || !musicaSelecionada) return
    setSalvandoCifra(true); setMsgCifra('')
    if (cifras[cifrasTom]) {
      await supabase.from('cifras').update({ conteudo: cifraTexto.trim() }).eq('id', cifras[cifrasTom].id)
    } else {
      await supabase.from('cifras').insert({ musica_id: musicaSelecionada.id, tom: cifrasTom, conteudo: cifraTexto.trim() })
    }
                  ...
                  <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:20 }}>
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:16 }}>
                        ...
                        {Object.keys(cifras).map(tom => (
                          <div key={tom} style={{ display:'flex', alignItems:'center', gap:8, background:'var(--bg3)', borderRadius:'var(--radius)', padding:'10px 14px' }}>
                            ...
                            <span style={{ flex:1, fontSize:12, color:'var(--text2)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{cifras[tom].conteudo.substring(0,60)}...</span>
                    ...
                    {cifraEditando !== null || true ? (
                      <form onSubmit={salvarCifra} style={{ display:'flex', flexDirection:'column', gap:12 }}>
                        ...
                        <div>
                          <label style={{ fontSize:11, color:'var(--text2)', display:'block', marginBottom:6, letterSpacing:1, textTransform:'uppercase' }}>Cifra</label>
                          <textarea value={cifraTexto} onChange={e => setCifraTexto(e.target.value)} placeholder="Cole a cifra aqui..." rows={10} style={{ width:'100%', fontFamily:'var(--font-mono)', fontSize:13, resize:'vertical', background:'var(--bg3)', border:'1px solid var(--border2)', color:'var(--text)', borderRadius:'var(--radius)', padding:12 }} />
                        </div>
                        ...
                        <div style={{ display:'flex', gap:8 }}>
                          <button type="submit" disabled={salvandoCifra || !cifrasTom || !cifraTexto.trim()} style={{ flex:1, padding:'11px 0', borderRadius:'var(--radius)', background: cifrasTom && cifraTexto.trim() ? 'var(--accent)' : 'var(--bg3)', color: cifrasTom && cifraTexto.trim() ? '#000' : 'var(--text3)', fontFamily:'var(--font-display)', fontWeight:700, fontSize:13, letterSpacing:2, border:'none', cursor:'pointer' }}>
                            {salvandoCifra ? 'SALVANDO...' : '✓ SALVAR CIFRA'}
                          </button>
                          {cifraEditando && <button type="button" onClick={() => { setCifraEditando(null); setCifraTexto(''); setCifrasTom('') }} style={{ padding:'11px 16px', borderRadius:'var(--radius)', background:'none', color:'var(--text3)', fontSize:12, border:'1px solid var(--border)', cursor:'pointer' }}>cancelar</button>}
                        </div>
                      </form>
                    ) : null}
                  </div>
