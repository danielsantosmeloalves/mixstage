...
export default function KitEnsaio({ session }) {
  ...
  async function aplicarPitch() {
    ...
    const currentOffset = wasPlaying ? ctx.currentTime - startedAtRef.current : offsetRef.current

    // 2. Som imediato com playbackRate (mantém iOS ativo durante processamento)
    if (wasPlaying || true) {
      iniciarSourceTemporaria(currentOffset, pitch)
      if (!wasPlaying) {
        // Se não estava tocando, para logo após iniciar (só para manter contexto)
        setTimeout(() => {
          if (!playingRef.current) {
            try { sourceRef.current?.stop() } catch(e) {}
            sourceRef.current = null
    ...
    // 3. Processa offline em background
    setProcessando(true); setProgressoProc(0)
    ...
    // 4. Troca para o buffer processado (pitch real sem alterar velocidade)
    if (ctx.state === 'suspended') await ctx.resume()
    const novoOffset = Math.max(0, ctx.currentTime - startedAtRef.current)
    const safeOffset = Math.min(novoOffset, novoBuffer.duration - 0.1)
