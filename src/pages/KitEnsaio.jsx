...
export default function KitEnsaio({ session }) {
  ...
  useEffect(() => {
    async function carregar() {
      const [mRes, kRes, cRes, tcRes, tfRes] = await Promise.all([
        supabase.from('musicas').select('*').eq('id', id).single(),
        supabase.from('kit_ensaio').select('*').eq('musica_id', id),
      ...
      setMusica(mRes.data)
      const mapa = {}
      if (kRes.data) kRes.data.forEach(item => { mapa[item.tipo] = item })
      ...
      if (tfRes.data) {
        ...
        setPitchSelecionado(tfRes.data.tom)
      }
      setCarregando(false)
    }
    carregar()
  ...
  useEffect(() => {
    ...
    if (fim && tempos.atual >= fim) seekTo(inicio)
  }, [tempos.atual])
  ...
  async function criarAudio() {
    ...
    const pitchShift = new Tone.PitchShift({ pitch: 0, windowSize: 0.03, delayTime: 0, feedback: 0, wet: 1 })
    // windowSize será ajustado dinamicamente conforme o pitch
  ...
  function seekTo(newOffset) {
    const safe = Math.max(0, Math.min(newOffset, durationRef.current - 0.1))
    offsetRef.current = safe
    setTempos(t => ({ ...t, atual: safe }))
    if (playingRef.current) {
      playerRef.current.stop()
      playerRef.current.start(undefined, safe)
  ...
  function seek(e) {
    ...
    seekTo(pct * durationRef.current)
  }
  ...
  async function aplicarPitch() {
    ...
    setProcessando(true)
    try {
      pitchShiftRef.current.windowSize = pitch < 0 ? 0.1 : 0.03
  ...
  function changeVolume(val) {
    ...
    setVolume(v)
    if (volumeRef.current) volumeRef.current.volume.value = Tone.gainToDb(v <= 0 ? 0.0001 : v)
  }

  function setupMediaSession() {
    if (!('mediaSession' in navigator)) return
    const tipoLabel = KIT_TIPOS.find(t => t.key === tipoSelecionado)?.label || ''
    navigator.mediaSession.metadata = new MediaMetadata({ title: musica?.titulo||'MixStage', artist: musica?.artista||'', album: `Kit Ensaio — ${tipoLabel}` })
  ...
  async function salvarTf() {
    ...
    const { error } = await supabase.from('tons_favoritos').upsert({ user_id:session.user.id, musica_id:id, tom:pitchAplicado }, { onConflict:'user_id,musica_id' })
        ...
        {!pronto && !precarregando && (
          <div className="fade-in" style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:'28px 24px', display:'flex', flexDirection:'column', gap:24 }}>
            ...
            <div>
              ...
              {tfSalvo !== null && (
                <button onClick={() => setPitchSelecionado(tfSalvo)} style={{ padding:'6px 12px', borderRadius:'var(--radius)', fontSize:11, marginBottom:10, background:pitchSelecionado===tfSalvo?'rgba(60,255,143,0.15)':'var(--bg3)', color:pitchSelecionado===tfSalvo?'var(--success)':'var(--text2)', border:`1px solid ${pitchSelecionado===tfSalvo?'rgba(60,255,143,0.3)':'var(--border)'}`, cursor:'pointer', letterSpacing:1 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', background:'var(--bg3)', borderRadius:'var(--radius)', border:'1px solid var(--border)' }}>
                ...
                <div style={{ flexShrink:0, textAlign:'center', minWidth:72 }}>
                  {tomSelecionadoDisplay ? (
                    ...
                    <div style={{ fontFamily:'var(--font-mono)', fontSize:14, color:pitchSelecionado===0?'var(--text2)':'var(--accent)' }}>{pitchSelecionado>0?'+':''}{pitchSelecionado} st</div>
                  )}
                </div>
              </div>
              {pitchSelecionado !== 0 && <button onClick={() => setPitchSelecionado(0)} style={{ marginTop:8, padding:'4px 10px', borderRadius:'var(--radius)', background:'none', color:'var(--text3)', fontSize:11, border:'1px solid var(--border)', cursor:'pointer' }}>↺ reset</button>}
            </div>
          ...
          <div className="fade-in" style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:'24px 20px' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20 }}>
                <button onClick={reiniciar} title="Voltar ao início" style={{ width:36, height:36, borderRadius:'var(--radius)', background:'var(--bg3)', border:'1px solid var(--border)', color:'var(--text2)', fontSize:16, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>⏮</button>
                <button onClick={() => pular(-5)} title="-5 segundos" style={{ width:36, height:36, borderRadius:'var(--radius)', background:'var(--bg3)', border:'1px solid var(--border)', color:'var(--text2)', fontSize:11, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-mono)' }}>-5s</button>
              ...
              <div style={{ padding:'14px 16px', background:'var(--bg3)', borderRadius:'var(--radius)', border:'1px solid var(--border)' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  {!processando && !pitchMudou && pitchAplicado!==0 && <button onClick={() => setPitch(0)} style={{ padding:'6px 12px', borderRadius:'var(--radius)', background:'none', color:'var(--text3)', fontSize:11, border:'1px solid var(--border)', cursor:'pointer' }}>↺ reset</button>}
                  <div style={{ flex:1 }} />
              ...
              {/* Time Code */}
              <TimeCodeNav secoes={secoes} tempoAtual={tempos.atual} onSeek={seekTo} onLoopChange={loop => { loopRef.current = loop }} />
