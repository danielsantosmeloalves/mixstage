...
export default function Mixer({ session }) {
  ...
  // AudioContext criado APENAS no Play — nunca antes
  const ctxRef = useRef(null)
  const masterRef = useRef(null)
  // ArrayBuffers brutos — não decodificados ainda
  const rawBuffersRef = useRef([])
  // AudioBuffers decodificados — preenchidos no primeiro Play
  ...
  // FASE 1: Carrega apenas os ArrayBuffers brutos (sem AudioContext)
  async function handleCarregar() {
    ...
    for (let idx = 0; idx < selecionadasArr.length; idx++) {
      ...
      try {
        ...
        console.warn('Erro ao baixar trilha', i, e)
      }
    }

    const validos = raws.filter(Boolean)
    if (validos.length === 0) {
      setErro('Não foi possível baixar as trilhas. Verifique sua conexão.')
      setPrecarregando(false)
      return
    }

    rawBuffersRef.current = raws
    audioBuffersRef.current = new Array(lista.length).fill(null)
  ...
  // FASE 2: Cria AudioContext e decodifica — chamado DENTRO do clique do Play
  async function decodificarEtocar() {
    // Cria AudioContext fresco dentro do gesto do usuário
    if (!ctxRef.current || ctxRef.current.state === 'closed') {
      const ctx = new (window.AudioContext || window.webkitAudioContext)()
      ctxRef.current = ctx
      masterRef.current = ctx.createGain()
      masterRef.current.connect(ctx.destination)
      gainNodesRef.current = rawBuffersRef.current.map(() => {
      ...
      decodificadoRef.current = false
      audioBuffersRef.current = new Array(rawBuffersRef.current.length).fill(null)
    }

    const ctx = ctxRef.current
    if (ctx.state === 'suspended') await ctx.resume()

    // Decodifica apenas se ainda não fez (ou contexto foi recriado)
    if (!decodificadoRef.current) {
      for (let i = 0; i < rawBuffersRef.current.length; i++) {
        const raw = rawBuffersRef.current[i]
        if (!raw) continue
        try {
          // Copia o ArrayBuffer pois decodeAudioData consome o original
          const copy = raw.slice(0)
          audioBuffersRef.current[i] = await ctx.decodeAudioData(copy)
        } catch(e) {
          console.warn('Erro ao decodificar trilha', i, e)
        }
      }
      const validos = audioBuffersRef.current.filter(Boolean)
      if (validos.length === 0) throw new Error('Nenhuma trilha decodificada')
  ...
  // Play/Pause — AudioContext criado AQUI, dentro do gesto
  async function togglePlay() {
    ...
    try {
      setPlaying(true) // feedback imediato
      await decodificarEtocar()
      iniciarSources(offsetRef.current)
      playingRef.current = true
      iniciarTick()
    } catch(e) {
      console.error('Erro ao tocar:', e)
      setErro('Erro ao iniciar áudio. Tente novamente.')
      setPlaying(false)
    }
  }
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'24px 16px' }}>
        ...
        {!pronto && !precarregando && (
          <div className="fade-in" style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:'32px 24px' }}>
            ...
            <div style={{ marginBottom:20 }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {trilhas.map((tr, i) => {
                  ...
                  return (
                    <div key={tr.id} onClick={() => setSelecionadas(prev => { const n = new Set(prev); sel ? n.delete(i) : n.add(i); return n })} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 14px', borderRadius:'var(--radius)', background: sel ? 'rgba(232,255,60,0.08)' : 'var(--bg3)', border:`1px solid ${sel ? 'rgba(232,255,60,0.3)' : 'var(--border)'}`, cursor:'pointer', transition:'all 0.15s' }}>
            ...
            <button onClick={handleCarregar} disabled={selecionadas.size === 0} style={{ width:'100%', padding:'16px 0', borderRadius:'var(--radius)', background: selecionadas.size > 0 ? 'var(--accent)' : 'var(--bg3)', color: selecionadas.size > 0 ? '#000' : 'var(--text3)', fontFamily:'var(--font-display)', fontWeight:700, fontSize:16, letterSpacing:3, border:'none', cursor: selecionadas.size > 0 ? 'pointer' : 'not-allowed' }}>
              ↓ BAIXAR {selecionadas.size > 0 ? `(${selecionadas.size})` : ''}
            </button>
          </div>
        )}
        ...
        {precarregando && (
          <div className="fade-in" style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:'32px 24px', textAlign:'center' }}>
            <div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:16, marginBottom:8 }}>Baixando trilhas...</div>
