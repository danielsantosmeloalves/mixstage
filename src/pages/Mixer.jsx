...
export default function Mixer() {
  ...
  async function handleCarregar() {
    ...
    const indiceSelecionadas = [...selecionadas].sort((a,b)=>a-b)
    let idx = 0
    for (let i = 0; i < lista.length; i++) {
      if (!selecionadas.has(i)) { buffers.push(null); continue }
      setTrilhaAtual(idx)
      idx++
      try {
        ...
        {!pronto && !precarregando && (
          <div className="fade-in" style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:'48px 24px', textAlign:'center' }}>
            ...
            <div style={{ color:'var(--text2)', fontSize:13, marginBottom:20 }}>
              Escolha quais faixas carregar:
            </div>

            {/* Seleção de faixas */}
            <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:24, textAlign:'left' }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                <button onClick={() => setSelecionadas(new Set(trilhas.map((_,i)=>i)))} style={{ fontSize:11, color:'var(--accent)', background:'none', border:'none', cursor:'pointer', letterSpacing:1 }}>
                  selecionar todas
                </button>
                <button onClick={() => setSelecionadas(new Set())} style={{ fontSize:11, color:'var(--text3)', background:'none', border:'none', cursor:'pointer', letterSpacing:1 }}>
                  limpar
                </button>
              </div>
              {trilhas.map((tr, i) => {
                ...
                return (
                  <div key={tr.id} onClick={() => setSelecionadas(prev => {
                    ...
                    <div style={{ width:3, height:20, borderRadius:1, background:['#e8ff3c','#3cffb0','#ff6b6b','#6bb3ff','#ff9f3c','#c46bff','#ff6bd6','#6bffd6'][i%8], flexShrink:0 }} />
                    <div style={{ flex:1, fontSize:13, color: sel ? 'var(--text)' : 'var(--text2)', fontFamily:'var(--font-display)', fontWeight:600 }}>
                      {tr.nome}
                    </div>
                    {deveExcluirPitch(tr.nome) && (
                      <div style={{ fontSize:9, color:'var(--text3)', background:'var(--bg2)', borderRadius:3, padding:'2px 5px', letterSpacing:1, flexShrink:0 }}>SEM PITCH</div>
                    )}
                  </div>
                )
              })}
