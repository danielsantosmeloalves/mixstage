...
// Limite automático baseado na RAM do dispositivo
function getLimiteTrilhas() {
  if (!IS_MOBILE) return 999
  const ram = navigator.deviceMemory || 4 // GB, padrão 4 se não suportado
  if (ram <= 2) return 8
  if (ram <= 4) return 12
  return 15
}
const LIMITE_MOBILE = getLimiteTrilhas()
...
export default function Mixer({ session }) {
  ...
  async function decodificarEConfigurar() {
    ...
    const pitchNode = new Tone.PitchShift({
      pitch: semitons,
      windowSize: semitons < 0 ? 0.1 : 0.03,
      delayTime: 0, feedback: 0, wet: 1,
    })
            ...
            <div style={{ textAlign:'center', marginBottom:24 }}>
              ...
              {IS_MOBILE && (
                <div style={{ fontSize:11, color:'var(--text3)', marginTop:10 }}>
                  Limite automático: {LIMITE_MOBILE} trilhas {navigator.deviceMemory ? `(${navigator.deviceMemory}GB RAM detectado)` : ''}
                </div>
              )}
            </div>
