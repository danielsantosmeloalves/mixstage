...
export default function Mixer({ session }) {
  ...
  // Pitch com compensação de velocidade:
  // playbackRate = 2^(semitons/12) muda pitch E velocidade
  // Para compensar a velocidade, usamos detune para pitch e
  // mantemos playbackRate = 1... mas detune também afeta velocidade.
  // Solução correta: playbackRate = 2^(semitons/12) para pitch,
  // e compensamos via um segundo nó com rate inverso.
  // Na prática mais simples e confiável: apenas detune nos sources.
  // O detune em cents = semitons * 100 afeta pitch sem mudar rate internamente,
  // mas em browsers reais ainda altera levemente.
  // A abordagem mais confiável cross-browser é:
  // - playbackRate = rateCompensado onde calculamos matematicamente a compensação

  function calcRate(semitons) {
    // playbackRate que só muda pitch sem alterar velocidade percebida
    // Usamos: pitch shift = 2^(s/12), speed compensation = 2^(-s/12)
    // Net effect: pitch muda, velocidade neutra
    // Na prática: detune = semitons * 100 cents (padrão Web Audio)
    return 1.0 // rate neutro — o pitch é feito via detune
  }
  ...
  function iniciarSources(offset) {
    ...
    // O detune (cents) nos browsers causa uma pequena variação de velocidade.
    // Para manter TODAS as trilhas em sincronia, aplicamos o mesmo detune
    // em TODAS — incluindo drums/click/guia. Isso garante que todas
    // acelerem/desacelerem exatamente igual, mantendo a sincronia perfeita.
    // A trilha "sem pitch" recebe detune=0 (tom original), mas o playbackRate
    // é sempre 1.0 em todas — sem diferença de velocidade entre elas.
    //
    // Correção de velocidade: detune em cents internamente aplica
    // playbackRate = 2^(cents/1200). Para compensar, aplicamos
    // playbackRate = 2^(-semitons/12) nas trilhas COM pitch,
    // fazendo com que a velocidade líquida = 1.0.
    // Trilhas SEM pitch: playbackRate = 1.0, detune = 0.
    // Resultado: todas na mesma velocidade, pitch diferente.

    const rateCompensacao = semitons !== 0 ? Math.pow(2, -semitons / 12) : 1.0
    ...
    const sources = bufRef.current.map((buf, i) => {
      ...
      if (semitons !== 0) {
        if (!excluir) {
          // Pitch shift: detune muda o tom
          src.detune.value = semitons * 100
          // Compensação de velocidade: cancela o efeito de velocidade do detune
          src.playbackRate.value = rateCompensacao
        } else {
          // Trilha sem pitch (drums, click, guia):
          // aplica apenas a compensação de velocidade para ficar em sincronia
          // com as trilhas que têm pitch+compensação
          // Rate líquido desejado = 1.0, então playbackRate = 1.0
          src.playbackRate.value = 1.0
          src.detune.value = 0
        }
      }
      // semitons === 0: tudo em 1.0, nada a fazer
                  ...
                  return (
                    <div key={tr.id} onClick={() => setSelecionadas(prev => {
                      ...
                      {deveExcluirPitch(tr.nome) && (
                        <div style={{ fontSize:9, color:'var(--text3)', background:'var(--bg2)', borderRadius:3, padding:'2px 5px', letterSpacing:1 }}>SEM PITCH</div>
                      )}
                    </div>
                  )
                })}
                  ...
                  <div key={tr.id} className="fade-in" style={{
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:2 }}>
                        ...
                        {deveExcluirPitch(tr.nome) && <div style={{ fontSize:9, color:'var(--text3)', background:'var(--bg3)', borderRadius:3, padding:'1px 5px', letterSpacing:1 }}>SEM PITCH</div>}
                      </div>
                    </div>
