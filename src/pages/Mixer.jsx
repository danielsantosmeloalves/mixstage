...
export default function Mixer({ session }) {
  ...
  function iniciarSources(offset) {
    ...
    const sources = bufRef.current.map((buf, i) => {
      ...
      if (!excluir && semitons !== 0) {
        // detune em cents: muda pitch. Para compensar velocidade,
        // aplicamos playbackRate inverso ao shift de velocidade causado pelo detune.
        // detune não afeta playbackRate — eles são independentes na spec.
        // Então: detune = semitons * 100 (pitch puro, sem mudar velocidade)
        src.detune.value = semitons * 100
        // playbackRate permanece 1.0 — velocidade não é afetada
      }
